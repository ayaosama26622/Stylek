import 'package:final_project/core/services/cart/cart_ids_controller.dart';
import 'package:final_project/core/services/cart/cart_key.dart';
import 'package:final_project/core/services/favorites/favorite_ids_controller.dart';
import 'package:final_project/core/services/favorites/favorite_key.dart';
...
class _ProductCardState extends State<ProductCard> {
  ...
  Future<void> _toggleFavorite(bool nextValue) async {
    try {
      await _homeCubit.toggleFavorite(widget.product, nextValue);
    } catch (error) {
  ...
  Widget build(BuildContext context) {
    ...
    return ListenableBuilder(
      listenable: FavoriteIdsController.instance,
      builder: (context, _) {
        final isFavorite = FavoriteIdsController.instance.isFavorite(
          favoriteKeyFor(product),
        );
...
class _AddToCartButtonState extends State<AddToCartButton> {
  Future<void> _toggleCart(bool nextValue) async {
    try {
      if (nextValue) {
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      listenable: CartIdsController.instance,
      builder: (context, _) {
        final isAdded = CartIdsController.instance.isInCart(
          cartKeyFor(widget.product),
        );
        return GestureDetector(
          onTap: () => _toggleCart(!isAdded),
          child: AnimatedContainer(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: isAdded ? AppColor.primaryPink : AppColor.surface(context),
                borderRadius: BorderRadius.circular(8.5),
              ),
              child: Icon(
                Icons.shopping_cart_outlined,
                size: 16,
                color: isAdded ? AppColor.darkColor(context) : AppColor.onSurface(context),
              ),
            ),
          ),
        );
